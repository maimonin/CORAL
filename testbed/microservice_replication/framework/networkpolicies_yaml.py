...
def create_networkpolicy(child_address, parents_ms_list):
    """
    ...
    if child_label_ms == "redis":
        ...
        child_ms_idx = child_label.split("-")[2]
    ...
    with open(
            f'C:/Users/Administrator/attack-graphs-local/attack-graphs-git/utils/netpol-template-selector.yaml',
            'r') as f_in:
        with open(
                f'C:/Users/Administrator/attack-graphs-local/attack-graphs-git/framework/yamls-to-apply/'
                f'{child_label_ms}-apply/{child_label_ms}-netpol-{child_ms_idx}.yaml', 'w') as f_out:
            child_netpol = yaml.load(f_in, Loader=SafeLoader)

            child_netpol['metadata']['name'] = f'{child_label_ms}-{child_ms_idx}-allow'

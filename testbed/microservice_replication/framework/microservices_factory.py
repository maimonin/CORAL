from frontend_yaml import *
from adservice_yaml import *
from productcatalogservice_yaml import *
from recommendationservice_yaml import *
from cartservice_yaml import *
from redis_cart_yaml import *
from checkoutservice_yaml import *
from shippingservice_yaml import *
from currencyservice_yaml import *
from paymentservice_yaml import *
from emailservice_yaml import *
from networkpolicis_yaml_separate_files import *


def validate(replications_dict):
    """
    Validates the relationships between the number of pods for parent and child microservices in a microservice
    architecture. Ensures that the number of child pods does not exceed the number of parent pods for each defined
    dependency. If any dependency rule is violated, an exception is raised.

    :param: replications_dict (dict): A dictionary mapping microservice names to the number of their respective pods.

    :raises: Exception: Raised if the number of pods for any child microservice exceeds the number of pods for its parent microservice.
    """
...
def add_frontend(num_replicates,
    ...
    """
    Creates (num_replicates) X new frontend microservices YAMLs.
...
def add_adservice(num_replicates):
    """
    Creates (num_replicates) X new adservice microservices YAMLs.
...
def add_productcatalogservice(num_replicates):
    """
    Creates (num_replicates) X new productcatalogservice microservices YAMLs.
...
def add_recommendationservice(num_replicates, producatcatalogservice_addresses):
    """
    Creates (num_replicates) X new recommendationservice microservices YAMLs.
...
def add_cartservice(num_replicates, redis_cart_addresses):
    """
    Creates (num_replicates) X new cartservice microservices YAMLs.
    Also updates used_ports list with the new ports allocated to the new cartservice services.
    This function also receives child microservices addresses', in order to write them to the parent microservice's (cartservice) YAMLs.

    :param num_replicates: int: number of replicates to create of cartservice microservice.
    :param redis_cart_addresses: dictionary: used and unused addresess of newly created (YAMLs) redis-cart microservices,
            which are cartservice' child microservices.

    :return: cartservice_addresess: dictionary: containing newly created cartservice's microservices' addresses.
    """
    cartservice_addresses = {"unused": [],
                             "used": []}

    for i in range(1, num_replicates + 1):
        new_cartservice_address = create_cartservice_yamls(idx=i, used_ports=used_ports,
                                                           child_to_parents_relations=child_to_parents_relations,
                                                           child_addresses_redis=redis_cart_addresses)
        cartservice_addresses['unused'].append(new_cartservice_address)

    return cartservice_addresses


def add_redis_cart(num_replicates):
    """
    Creates (num_replicates) X new redis-cart microservices YAMLs.
    Also updates used_ports list with the new ports allocated to the new redis-cart services.

    :param num_replicates: int: number of replicates to create of redis-cart microservice.

    :return: redis_cart_addresess: dictionary: containing newly created redis-cart's microservices' addresses.
    """
    redis_cart_addresses = {"unused": [],
                            "used": []}

    for i in range(1, num_replicates + 1):
        new_redis_cart_address = create_redis_cart_yamls(idx=i, used_ports=used_ports,
                                                         child_to_parents_relations=child_to_parents_relations)
        redis_cart_addresses['unused'].append(new_redis_cart_address)

    return redis_cart_addresses
...
def add_checkoutservice(num_replicates, producatcatalogservice_addresses, cartservice_addresses,
    ...
    """
    Creates (num_replicates) X new checkoutservice microservices YAMLs.
...
def add_shippingservice(num_replicates):
    """
    Creates (num_replicates) X new shippingservice microservices YAMLs.
...
def add_currencyservice(num_replicates):
    """
    Creates (num_replicates) X new currencyservice microservices YAMLs.
...
def add_paymentservice(num_replicates):
    """
    Creates (num_replicates) X new paymentservice microservices YAMLs.
...
def add_emailservice(num_replicates):
    """
    Creates (num_replicates) X new emailservice microservices YAMLs.
...
def replicate_microservices(replications_dict):
    """
    Validates and replicates various microservices as specified in the input dictionary.
    The function initializes and replicates microservices according to their respective keys within the input dictionary.
    Additionally, it handles non-replicable, hard-coded microservices.
    For all specified services, respective addresses and relations are updated.

    :param: replications_dict: dict
            A dictionary where each key is a microservice name and its value is the number of replicates required.
    ...
    # redis_cart_addresses = add_redis_cart(replications_dict['redis-cart'])
    # cart_addresses = add_cartservice(num_replicates=replications_dict['cartservice'],
    #                                  redis_cart_addresses=redis_cart_addresses)
    # Copy hard-coded yamls to YAMLs 'apply' directory.
...
def generate_networkpolicies(child_to_parents_relations):
    """
    This function iterates through the child_to_parents_relations dictionary and for each child microservices
    creates network policies according to each parent microservice.
